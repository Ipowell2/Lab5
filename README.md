...
Normal Course: 
The user selects "Log In" on the homepage.
The system prompts the user to enter their username/email and password.
The user inputs their credentials and clicks the "Submit" button.
The system verifies the credentials against the stored user data in the database.
If the credentials are correct, the system logs the user in and redirects them to their dashboard.
The dashboard displays the user’s portfolio, available virtual funds, and recent market data.
Postconditions 
...
Normal Course: 
User enters login credentials (username and password) and submits the form.
System validates credentials and, if correct, prompts the user for 2FA.
System generates a one-time code and sends it to the user's email or phone.
User receives the code and enters it in the system's 2FA prompt.
System verifies the entered code within the allowed time limit.
...
Use Case 3
Use Case Name: Depositing User Funds
ID: UC-3 
Priority: High
...
Normal Course: 
 User navigates to the account or wallet section of the platform.
User selects the option to deposit funds.
System prompts the user to enter the amount they wish to deposit (virtual funds for simulation or real currency, depending on system setup).
User selects a payment method or confirms virtual funds transfer.
...
System confirms the deposit and updates the user's account balance.
User receives a confirmation message that the funds have been successfully deposited.
...
Use Case 4
Use Case Name: Simulated Stock Trading
ID: UC-4
Priority: High
...
Normal Course: 
The user navigates to the "Trading Dashboard."
The system displays available stocks with real-time data, including prices.
The user selects a stock they wish to buy or sell.
The system prompts the user to enter the number of shares and the type of transaction (buy/sell).
The user confirms the transaction.
The system checks if the user has sufficient funds (for buy) or the required shares (for sell).
If valid, the system processes the transaction:
...
The system updates the user's transaction history and portfolio.
The user receives a confirmation message that the transaction has been successfully completed.
Postconditions 
...
Use Case 5
Use Case Name: Portfolio Management
ID: UC-5
Priority: Medium
...
Normal Course: 
The user navigates to the "Portfolio" section on the dashboard.
The system retrieves the user's portfolio details from the user database.
The system fetches real-time stock prices from the Stock Data API.
The portfolio is displayed with updated stock prices, holdings, and available funds.
The user can select the "Transaction History" tab to view a list of past trades, which is retrieved from the user database.
The system displays the user's transaction history.
Postconditions 
